...
export async function generateSitePayrollExcel(report: SitePayrollReport) {
  ...
  worksheet.views = [
    { state: "frozen", xSplit: 0, ySplit: worksheet.rowCount },
  ];
...
export async function generateKWDIReportExcel(report: SitePayrollReport) {
  ...
  report.entries.forEach((entry, index) => {
    ...
    const nationalityCode = entry.is_foreign
  ...
  const guideData = [
    ...
    [
      2,
      "성명",
      "주민등록표(외국인등록증 또는 국내거소신고증)상의 성명을 적습니다.",
    ],
...
export async function generateNationalTaxExcel(report: SitePayrollReport) {
  ...
  report.entries.forEach((entry, index) => {
    ...
    const row = worksheet.addRow([
      ...
      entry.phone ?? "", // E: 전화번호
      monthStr, // F: 지급월 (MM)
      entry.nts_work_month ? entry.nts_work_month.slice(-2) : monthStr, // G: 근무월 (MM)
