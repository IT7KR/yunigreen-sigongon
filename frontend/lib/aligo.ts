...
가입 완료 후 근무 배정 및 급여 지급이 가능합니다.`,
  ...
  WORKER_REGISTERED: {
    ...
    name: "근로자 가입 완료",
    content: `[시공코어] #{name}님, 가입이 완료되었습니다.
...
근무 배정 및 급여 내역은 앱에서 확인하세요.`,
  ...
  USER_INVITE: {
    ...
    name: "사용자 초대",
    content: `[시공코어] #{name}님, 안녕하세요.
#{inviter_name}님이 시공코어 사용을 초대했습니다.
...
아래 링크에서 가입을 완료해주세요.`,
  ...
  CONTRACT_SIGN: {
    code: "CONTRACT_SIGN",
    name: "계약서 서명 요청",
    content: `[시공코어] #{name}님, 근로계약서 서명을 요청합니다.
...
아래 링크에서 계약 내용 확인 후 서명해주세요.`,
  ...
  PAYSTUB_SENT: {
    code: "PAYSTUB_SENT",
    name: "급여명세서 발송",
    content: `[시공코어] #{name}님, #{year}년 #{month}월 급여명세서입니다.
 ...
 * 알림톡 발송 상태 조회
 */
export async function getAlimTalkStatus(
  request: AlimTalkStatusRequest,
): Promise<AlimTalkStatusResponse> {
  const useMocks = process.env.NEXT_PUBLIC_USE_MOCKS === "true";
