...
export class MockAPIClient {
  ...
  async sendLaborContractForSignature(laborContractId: string) {
    return delay(ok({ id: laborContractId, status: "sent", signature_url: `mock://labor-sign/${laborContractId}` }))
  }
  ...
  async updateUtilityStatus(_projectId: string, _utilityId: string, _data: { status?: "pending" | "completed"; doc_status?: "pending" | "submitted" }) {
    this.ensureUtilities(_projectId)
