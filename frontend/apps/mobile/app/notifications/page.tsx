...
import { Card, CardContent } from "@yunigreen/ui"
    ...
    <MobileLayout title="알림" showBack>
      <div className="divide-y divide-slate-100">
        {isLoading ? (
          ...
          <div className="p-4 text-center text-sm text-slate-400">알림이 없습니다.</div>
        ) : (
          notifications.map((noti) => (
            <button
              ...
              className={`flex w-full gap-4 p-4 text-left ${noti.read ? "bg-white" : "bg-teal-50/30"}`}
            >
              <div
                className={`mt-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full ${
                  noti.type === "contract" ? "bg-blue-100 text-blue-600" : "bg-teal-100 text-teal-600"
                }`}
              >
                {noti.type === "contract" ? (
              ...
              <div className="flex-1">
                <div className="flex justify-between">
                  <p className={`font-medium ${noti.read ? "text-slate-900" : "text-teal-900"}`}>
                    {noti.title}
                  </p>
