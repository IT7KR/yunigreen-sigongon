    ...
    <div className="flex min-h-screen flex-col bg-slate-50">
      ...
      <main className="flex-1 p-4 space-y-4">
        {isLoading ? (
          ...
          <div className="text-center text-sm text-slate-400">지급명세서가 없습니다.</div>
        ) : (
          paystubs.map((item) => (
            <Link key={item.id} href={`/worker/paystubs/${item.id}`}>
              <Card>
                <CardContent className="flex items-center justify-between p-4">
                  <div>
                    <h3 className="font-bold text-slate-900">{item.month} 급여</h3>
                    <p className="text-sm text-slate-500">지급일: {item.date}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-teal-600">{item.amount.toLocaleString()}원</p>
                    <p className="text-xs text-slate-400">
                      {item.status === "confirmed" ? "수령확인완료" : "미확인"}
                    </p>
                  </div>
                </CardContent>
              </Card>
