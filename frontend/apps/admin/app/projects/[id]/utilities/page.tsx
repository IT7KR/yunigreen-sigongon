...
export default function UtilitiesPage({
  ...
  >([])
  const [timeline, setTimeline] = useState<Array<{ id: string; date: string; message: string }>>([])
  ...
  const handleToggle = async (itemId: string, status: "pending" | "completed") => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900">수도광열비 관리</h2>
        <Button>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {isLoading ? (
          <Card>
            <CardContent className="p-6 text-center text-sm text-slate-400">불러오는 중...</CardContent>
          </Card>
        ) : utilities.length === 0 ? (
          <Card>
            <CardContent className="p-6 text-center text-sm text-slate-400">등록된 항목이 없습니다.</CardContent>
          </Card>
        ) : (
          utilities.map((item) => (
            <Card key={item.id}>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{item.amount.toLocaleString()}원</div>
              <p className="text-xs text-slate-500">납부기한: {item.due_date}</p>
              
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">공문 발송</span>
                  <Badge variant={item.doc_status === "submitted" ? "success" : "warning"}>
                    {item.doc_status === "submitted" ? "발송 완료" : "발송 대기"}
                  </Badge>
                </div>
                <div className="flex items-center justify-between text-sm">
