...
export default function WorkflowPage({
  ...
  const steps = [
    ...
    },
    {
      ...
      status: project.status === "completed" ? "current" : "pending",
      action: {
        label: "준공 서류 작성",
                ...
                <div key={step.id} className="relative">
                  <div
                    className={cn(
                      ...
                      isCompleted
                        ? "border-brand-point-500 text-brand-point-500"
                        : isCurrent
                          ? "border-brand-point-500 text-brand-point-500"
                  ...
                  >
                    {isCompleted ? (
                      <CheckCircle2 className="h-4 w-4 fill-brand-point-50" />
                    ) : (
                      <Circle
                        className={cn(
                          "h-3 w-3 fill-current",
                          isCurrent ? "text-brand-point-500" : "text-transparent",
                        )}
                      />
                    )}
                  </div>
                  ...
                  <div
                    className={cn(
                      ...
                      isCurrent
                        ? "border-brand-point-200 bg-brand-point-50/50"
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <h3
                          className={cn(
                            "font-semibold",
                            isCurrent ? "text-brand-point-900" : "text-slate-900",
                          )}
                        >
                          {step.title}
                          {isCurrent && (
                            <span className="ml-2 inline-flex items-center rounded-full bg-brand-point-100 px-2 py-0.5 text-xs font-medium text-brand-point-800">
                              진행 중
                            </span>
                          )}
                        </h3>
