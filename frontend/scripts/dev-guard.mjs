 ...
 * 
 * Only kills processes that match our yunigreen project path.
 */

import { execSync } from 'child_process';

const DEV_PORTS = [3033, 3034]; // admin, mobile
const PROJECT_IDENTIFIER = 'yunigreen-dev/frontend';
...
function getProcessOnPort(port) {
  try {
    ...
    if (output) {
      ...
      try {
        const cmdline = execSync(`ps -p ${pid} -o args= 2>/dev/null`, { encoding: 'utf-8' }).trim();
...
function isOurProcess(processInfo) {
  ...
  try {
    const cwd = execSync(`readlink /proc/${processInfo.pid}/cwd 2>/dev/null`, { encoding: 'utf-8' }).trim();
    return cwd.includes(PROJECT_IDENTIFIER);
  } catch {
    // /proc not available (macOS) - check via lsof
    try {
      const lsofOutput = execSync(`lsof -p ${processInfo.pid} 2>/dev/null | grep cwd`, { encoding: 'utf-8' });
...
for (const port of DEV_PORTS) {
  ...
  if (isOurProcess(processInfo)) {
    console.log(`  \x1b[33m[CLEANING]\x1b[0m Port ${port} has stale yunigreen process (PID ${processInfo.pid})`);
    if (killProcess(processInfo.pid)) {
      console.log(`    \x1b[32m[KILLED]\x1b[0m Successfully cleaned up PID ${processInfo.pid}`);
    } else {
      console.log(`    \x1b[31m[FAILED]\x1b[0m Could not kill PID ${processInfo.pid}`);
      hasErrors = true;
    }
  } else {
    console.log(`  \x1b[31m[BLOCKED]\x1b[0m Port ${port} is in use by another process:`);
