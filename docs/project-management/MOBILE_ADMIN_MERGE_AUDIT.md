...
- **공유 패키지** (7개) — `@sigongon/ui`, `@sigongon/types`, `@sigongon/platform`, `@sigongon/api`, `@sigongon/mocks`, `@sigongon/queries`, `@sigongon/features`
...
| # | 파일 | 사유 |
|---|------|------|
| 1 | `lib/api.ts` | `@sigongon/api` 공유 패키지 사용 |
| 2 | `lib/auth.tsx` | `@sigongon/platform` createAuthMiddleware 공유 |
...
현장 사진 촬영은 시공ON의 핵심 현장 기능이며, 이 모듈 없이는 일일보고서, 준공 앨범, 현장 기록이 불가능.
...
- [ ] **사진 촬영 페이지 생성** (`/projects/[id]/photos`)
  ...
  - [ ] 프로젝트 상세 탭에 "사진" 링크 추가
- [ ] **전자서명 패드 동작 검증**
  - [ ] `@sigongon/ui` SignaturePad 컴포넌트 터치/마우스 동작 확인
