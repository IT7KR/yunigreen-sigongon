...
| **수의계약체결제한여부확인서** | HWP→PDF | 템플릿 기반 | 사업자 정보 기반 자동 작성 |
| **시방서** | PDF | 시스템 자동 생성 | 선정된 공종 기반 자동 생성 |
| **사업자 외 서류** | PDF | 사용자 업로드 | 회원가입 시 등록한 서류(사업자등록증, 통장사본 등) 활용 |
| **계약보증서** | PDF | 사용자 업로드 | 전문건설공제조합 발급 보증서 또는 지급각서 |
| **계약보증금 지급각서** | HWP→PDF | 템플릿 기반 | 보증서 대체 시 사용. 사업자 정보 자동 삽입 |
...
| 문서명 | 출력 형식 | 생성 방식 | 설명 |
|--------|---------|---------|------|
| **착공공문** | HWP→PDF | 템플릿 기반 | 발주자/건물주 정보 자동 삽입 |
| **착공신고서** | PDF | 템플릿 기반 | 건설업 신고(발주청) 또는 관공서 양식 |
| **현장대리인 서류** | PDF | 사용자 업로드 | 기술자격 증명서, 보험료 영수증 |
| **계약내역서** | Excel | 시스템 자동 생성 | Phase 1의 견적내역서와 동일 |
| **노무비 서류** | PDF | 템플릿 기반 | 상용근로자/일용근로자 중 선택 |
| **안전보건 서약서** | PDF | 템플릿 기반 | 안전관리자 및 담당자 서명 |
...
| **일용근로계약서** | PDF | 시스템 자동 생성 | 근로자별 일일 계약서. 사인 이미지 저장 |
...
| 문서명 | 출력 형식 | 생성 방식 | 설명 |
|--------|---------|---------|------|
| **준공공문** | HWP→PDF | 템플릿 기반 | 발주자/건물주 통지 |
| **준공계** | PDF | 템플릿 기반 | 공사 기본 정보(기간, 금액, 공종 등) 자동 삽입 |
| **준공내역서** | Excel | 시스템 자동 생성 | Phase 1 견적내역서 변동 반영 |
| **준공정산동의서** | PDF | 템플릿 기반 | 조건부 발생. 공사금액 변경 시 작성 |
...
| **수도/전기 공문** | PDF | 템플릿 기반 | 수도/전기 회사 통보용 |
...
| **시방서** | PDF | DOCX | 한글 원본은 별도 검토 |
| **공문/신고** | PDF | - | 정부 제출용 기본 형식 |
| **일용근로계약** | PDF | Excel | 인쇄/보관 위해 PDF 기본 |
| **공사일지** | PDF, Excel | - | 날짜별 조회 시 Excel |

**한글(.hwp) 파일 생성 불가 사유**:
- 웹 환경에서 한글 파일 생성 기술적 제약
- hwpx 표준화 미흡
- PDF/DOCX로 충분한 호환성
