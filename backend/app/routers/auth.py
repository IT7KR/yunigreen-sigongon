...
async def verify_business(
    ...
    """사업자 통합 검증 - DB 중복 확인 + 국세청 API 연동.

    1. DB 중복 확인 - 이미 가입된 사업자번호이면 duplicate 반환
    2. 사업자 상태 조회 (verify) - active/suspended/closed/unknown 반환
    """
    ...
    business_status = status_map.get(verify_result.status_code, "unknown")

    return APIResponse.ok({
        "status": business_status,
        "tax_type": verify_result.tax_type if business_status == "active" else None,
    })
